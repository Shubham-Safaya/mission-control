...
- **identity-resolver is NOT on PyPI** (both names 404). The "on PyPI" claim is currently untrue everywhere it appears. Fix tonight: `cd identity-resolution-engine && python -m build && twine upload dist/*` (PyPI account + token needed). Portfolio stat shows "—" until then, by design.
...
## DEFERRED (build-ready plans)
| Item | Why deferred | The plan |
|---|---|---|
| 7b synthetic 100K identity-graph demo | Depends on the PyPI publish; biggest remaining build | Generate population from PUMS distributions → `pip install identity-resolver` → waterfall match → precision/recall vs manufactured truth → household inspector page. All patterns exist in Checkpoint/pulse code. |
...
| yt-shorts-studio v2 (P2) | Heavy local pipeline | yt-dlp → faster-whisper → heuristic highlight scoring → ffmpeg 9:16 + ASS captions; repo already has v1 plans engine. |
| Polish sweep (P3.1) | Most repos already meet standard from prior sessions | Remaining: ThreadPass product-thesis page, footer consistency pass. |
| daily-log auto-stub (P3.2) | Repo already runs a morning generate Action | Extend `generate_log.py` with commits/PyPI deltas + BOOK.md ToC. |
